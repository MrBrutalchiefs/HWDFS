...
Setting* RSConfiguration::load(const char * fileName) {
    ...
    // 获取更新策略
    elem = elem->NextSiblingElement();
    TiXmlElement* scheme_elem = elem->FirstChildElement()->FirstChildElement();
    string updateScheme = scheme_elem->GetText();
    Setting* setting = new Setting(dataChunk, parityChunk, chunkSize, partSize, updateScheme);

    // 获取Master服务器的IP地址
    elem = elem->NextSiblingElement();
    TiXmlElement* master_elem = elem->FirstChildElement();
    string master_ip = master_elem->GetText();
    master_elem = master_elem->NextSiblingElement();
    int master_port = atoi(master_elem->GetText());

    // 初始化Master服务器
    std::shared_ptr<TTransport> socket(new TSocket(master_ip, master_port));
    std::shared_ptr<TTransport> transport(new TBufferedTransport(socket));
    std::shared_ptr<TProtocol> protocol(new TBinaryProtocol(transport));
    setting->master = new MasterServiceClient(protocol);
