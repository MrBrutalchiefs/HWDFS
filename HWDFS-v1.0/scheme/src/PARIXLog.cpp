...
void PARIXLog::writeData(Response& _return, const Request& request, Buffer* buffer, Setting* setting) {
    TSCNS ts;
    ts.init();
...
void PARIXLog::writeParity(Response& _return, const Request& request, Buffer* buffer, Setting* setting) {
...
void PARIXLog::readData(Response &_return, const Request &request, Buffer* buffer, Setting* setting) {
    if(request.type == comm::DataType::D0) {
        char * temp = new char[request.length];
        Manage::readData(setting->servers[request.to_id]->path + "/user_data.bin",
...
void PARIXLog::readParity(Response& _return, const Request& request, Buffer* buffer, Setting* setting) {
...
void PARIXLog::degrade_read(Response& _return, const Request& request, Buffer* buffer, Setting* setting) {
    // 采用日志重播的方式恢复dr
    Request req(request);
    req.from_id = request.to_id;
...
void PARIXLog::mergeData(Response &_return, const Request &request, Buffer* buffer, Setting* setting) {
    char * temp = new char[setting->part];
...
void PARIXLog::mergeParity(Response& _return, const Request& request, Buffer* buffer, Setting* setting) {
    char* dr = new char[setting->part];
    char* d0 = new char[setting->part];
