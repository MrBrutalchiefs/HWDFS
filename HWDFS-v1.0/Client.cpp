...
int main(int argc, char* argv[]) {
    if(string(argv[1]) == "-h") {
        ...
        manage = new MetaManage((config_path + "/" + string(argv[1])).c_str());
        if(string(argv[2]) == "-c") {
            create();
...
void create() {
    int chunkNum = manage->manage->setting->data;
    unsigned long length = chunkNum * manage->manage->setting->size;
...
void update_sample() {
    // 加载trace文件
    vector<Trace> traces;
    int64_t segSize = manage->manage->setting->data * manage->manage->setting->size;
...
void update_iops(int rounds, int num, int64_t update_size) {
    int64_t offsets[num];
    int64_t step = (manage->manage->setting->data * manage->manage->setting->size) / num;
    for (int i = 0; i < num; ++i) {
        offsets[i] = i * step;
    }
